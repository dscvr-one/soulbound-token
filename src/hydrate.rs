...
#[pre_upgrade]
fn pre_upgrade() {
    ic_cdk::println!("[SNS-1 Token] Pre Upgrade Started!");
    State::mutate_state(|s| stable_save(StableStorage::from(s)).unwrap());
    ic_cdk::println!("[SNS-1 Token] Pre Upgrade Finished");
}

#[post_upgrade]
fn post_upgrade() {
    ic_cdk::println!("[SNS-1 Token] Post Upgrade Started!");
    State::mutate_state(|s| {
        let (storage, instructions) = stable_restore::<StableStorage, u64>();
        let storage = storage.unwrap();
        let pre_upgrade_count = instructions.unwrap();
        ic_cdk::println!(
            "[SNS-1 Token] Total Pre Upgrade Instruction Count: {:?}",
            pre_upgrade_count
        );
        *s = storage.into();
        let count = ic_cdk::api::instruction_counter();
        ic_cdk::println!("[SNS-1 Token] Total Post Upgrade Instruction Count: {:?}", count);
        ic_cdk::println!(
            "[SNS-1 Token] Total Pre + Post Upgrade Instruction Count: {:?}",
            pre_upgrade_count + count
        );
    });
    ic_cdk::println!("[SNS-1 Token] Post Upgrade Finished");
}
